...
Please complete and submit this Rmd by emailing it to either yzh191@u.rochester.edu or omohan@ur.rochester.edu by **11:59 pm** on **Aug 21, 2022**. Make sure it knits without errors before submission. A good way to check whether you have filled out all the blanks is by looking for all the <\br>s and make sure they have been replaced with the appropriate answers. You are welcome to edit the author and date on top of this document. When encountering any doubts and questions on coding, we encourage you to post them on Slack. Don't waste hours of your time on being stalled. Especially during first few weeks of learning R and R markdown in parallel to learning statistics, we would like to make sure that we minimize any issues that are due to learning a new programming environment. We also encourage all of you to try to answer questions on Slack. If we all just try, we will learn from each other, including from our mistakes. 
...
1. **Summarizing Data.** Fill the following two code chunks, 1) to visualize and 2) to calculate and fill out following table on the statistical summary of the generated [sample data](#data)
```{r}
...
4. **Simulation-based Approach.** As an primer for our Week 1 Simulation, we can try a simulation-based approach to understanding p-values, by generating samples from the population distributions 1000 times and visualizing the p-values obtained. 
...
What do you expect to see (in the distribution of 1000 p-values) when the means and SDs of the cats and dogs are the same? : <\br>

Make corresponding changes to the code chunk above to check your prediction. 
What do you see and what does it mean: <\br> 
